    ...
    public class WsBarrageService
    {
        ...
        private MsgUser GetUser(User data)
        {
            MsgUser user = new MsgUser()
            {
                ...
                HeadImgUrl = data?.avatarThumb?.urlLists,
                SecUid = data?.sec_uid,
        ...
        private void PrintMsg(Msg msg, PackMsgType barType)
        {
            ...
            var roomName = (rinfo?.Owner?.Nickname ?? ("直播间" + (msg.WebRoomId == -1 ? msg.RoomId : msg.WebRoomId)));
            ...
            switch (barType)
            {
                case PackMsgType.弹幕消息:
                    ...
                    append = $"{msg?.User?.Nickname}: {msg.Content}";
                    break;
